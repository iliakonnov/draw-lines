...
namespace gui
{
	public class ImageDialog : Dialog
	{
		...
		void Save_Btn_Click(object sender, EventArgs e)
		{
			...
			var fName = save_dialog.FileName;
			if (fName != null)
			{
		...
		public ImageDialog(Image img)
		{
			...
			if (img.Width > gui.maxWidth || img.Height > gui.maxHeight)  // Big image
			{
				...
				var bmImg = new Bitmap(img, size.Width, size.Height, ImageInterpolation.Default);
				...
				img = bmImg;
				imgSize_label.Text += string.Format(" (preview is resized to {0}x{1})", size.Width, size.Height);
			}
